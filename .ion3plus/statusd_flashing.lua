...
local defaults = {
	...
	update_interval = 3*1000,    --> Check your files every X milliseconds.
	flash_interval = 400,        --> Blinking interval in milliseconds.
	turn_off = 20,		     --> Turn Off the alarm if it annoys you too much time: 
...
local message_length = string.len(settings.alarm_message)
...
local function flash_alarm()
	...
	local hide = function()
		statusd.inform("flashing", string.rep(".", message_length))
		return false
	end
