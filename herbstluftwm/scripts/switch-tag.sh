...
# EDIT FROM ORIGINAL:
# Removed '!' (urgent window flag) from `checkuse' unused tag check.
# Avoid switching to the tag defined in ${skiptag}

skiptag="X"

checkuse() {
    if [[ "${tags[$1]}" != [.]* ]] && [[ "${tags[$1]}" != *${skiptag} ]]; then # tag has open client(s) and is NOT tag "skiptag"
...
# do nothing if focused index is "skiptag"
if [[ ${activetag} = $(herbstclient attr tags.by-name.${skiptag}.index) ]]; then
    exit 0
fi
