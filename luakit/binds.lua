...
local scroll_step_more = globals.scroll_step_more or 200
local scroll_step_micro = globals.scroll_step_micro or 2
...
add_binds("normal", {
    ...
    key({}, "J", "Scroll document down, further",
...
--]]
    ...
    key({}, "Down", "Scroll document down.",
        function (w) w:scroll{ yrel =  scroll_step_micro } end),

    key({}, "Up",   "Scroll document up.",
        function (w) w:scroll{ yrel = -scroll_step_micro } end),

    key({}, "Left", "Scroll document left.",
        function (w) w:scroll{ xrel = -scroll_step_micro } end),
...
add_cmds({
    ...
    cmd("q[uit]", "Close the current tab or window if no more tabs.",
        function (w, a, o) if w.tabs:count() > 1 then w:close_tab() else w:close_win(o.bang) end end),

    cmd("qa", "Close the current window without saving session.",
