...
(defvar *frequent-map*
  (let ((m (make-sparse-keymap)))
    (labels ((dk (m k c) (define-key m k c)))
;    (dk m (kbd "2") "exec volpulse speakers")
;    (dk m (kbd "3") "exec volpulse headphones")
    ...
    (dk m (kbd "s-b") "exec display -window root -resize 1600x900! /howl/foto/wall/smoking_indian_1.png")
    (dk m (kbd "g") "exec gimp -s --stack-trace-mode=never")
    (dk m (kbd "h") "exec urxvt -e htop")
    (dk m (kbd "l") "ror_luakit")
;    (dk m (kbd "m") "exec volpulse toggle")
    (dk m (kbd "m") "exec volpulse mute")
    (dk m (kbd "M") "exec volpulse unmute")
...
(defvar *mplayer-map1*
  (let ((m (make-sparse-keymap)))
    (labels ((dk (m k c) (define-key m k c)))
    (dk m (kbd "0")     "exec mifo --reload list")
    (dk m (kbd "1")     "exec mifo --reload song")
    (dk m (kbd "2")     "exec mifo --reload last")
    ...
    (dk m (kbd "h")     "exec mifo -p")
    (dk m (kbd "H")     "prompt-mifo-prev")
    (dk m (kbd "j")     "prompt-mifo-next +")
    (dk m (kbd "k")     "prompt-mifo-prev +")
    (dk m (kbd "l")     "exec mifo -n")
    ...
    ;(dk m (kbd "Q")     "exec sudo /etc/rc.d/mifo purge")
...
;    (dk m (kbd "XF86AudioMute")         "exec volpulse toggle")
    (dk m (kbd "XF86AudioRaiseVolume")  "exec volpulse up")
    (dk m (kbd "XF86AudioLowerVolume")  "exec volpulse down")
;    (dk m (kbd "XF86Sleep")             "exec sudo pm-suspend")
   M)))
