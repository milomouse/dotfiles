...
(defvar *frequent-map*
  (let ((m (make-sparse-keymap)))
    ...
    (dk m (kbd "B") "exec display -window root -resize 1366x768! /howl/foto/wall/1366x768-dizorb-breakdown-the-space-hd-wallpaper.png")
    (dk m (kbd "C-b") "exec display -window root -resize 1366x768! /howl/foto/wall/1366x768_nasa-2010-1674.png")
    (dk m (kbd "M-b") "exec display -window root -resize 1366x768! /howl/foto/wall/1366x768_dizorb_landscape_study_hd_wallpaper.png")
    (dk m (kbd "s-B") "exec display -window root -resize 1366x768! /howl/foto/wall/wallpaper-249719.png")
...
(setf *top-map*
  (let ((m (make-sparse-keymap)))
    ...
    (dk m (kbd "s-Return")          "exec urxvt -e tmux -f ${XDG_CONFIG_DIR:-${HOME}}/tmux/tmux.conf -L xorg new-session")
    ...
    (dk m (kbd "s-S-Delete")        "exec alock -bg image:file=${XDG_PICTURES_DIR:-${H:-/howl}/foto}/wall/beheading.png -cursor glyph -auth pam >&/dev/null")
    ;; <alphabetic bindings>
    ...
    (dk m (kbd "s-C-F1")   "dump-group-to-file /howl/conf/stumpwm/storage/group_1")
    (dk m (kbd "s-C-F2")   "dump-group-to-file /howl/conf/stumpwm/storage/group_2")
    (dk m (kbd "s-C-F3")   "dump-group-to-file /howl/conf/stumpwm/storage/group_3")
    (dk m (kbd "s-C-F4")   "dump-group-to-file /howl/conf/stumpwm/storage/group_4")
    (dk m (kbd "s-C-F5")   "dump-group-to-file /howl/conf/stumpwm/storage/group_5")
    (dk m (kbd "s-C-F6")   "dump-group-to-file /howl/conf/stumpwm/storage/group_6")
    (dk m (kbd "s-C-F7")   "dump-window-placement-rules /howl/conf/stumpwm/storage/placement_rules")
    (dk m (kbd "s-C-F9")   "dump-screen-to-file /howl/conf/stumpwm/storage/screen_data")
    (dk m (kbd "F1")     "restore-from-file /howl/conf/stumpwm/storage/group_1")
    (dk m (kbd "F2")     "restore-from-file /howl/conf/stumpwm/storage/group_2")
    (dk m (kbd "F3")     "restore-from-file /howl/conf/stumpwm/storage/group_3")
    (dk m (kbd "F4")     "restore-from-file /howl/conf/stumpwm/storage/group_4")
    (dk m (kbd "F5")     "restore-from-file /howl/conf/stumpwm/storage/group_5")
    (dk m (kbd "F6")     "restore-from-file /howl/conf/stumpwm/storage/group_6")
    (dk m (kbd "F7")     "restore-window-placement-rules /howl/conf/stumpwm/storage/placement_rules")
    (dk m (kbd "F9")     "restore-from-file /howl/conf/stumpwm/storage/screen_data")
    (dk m (kbd "F10")    "restore-from-file /howl/conf/stumpwm/storage/desktop_data")
